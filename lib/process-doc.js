...
module.exports = function (markdownData, isBuild, noPreview, babelConfig) {
  ...
  if (demoConfig) {
    markdownData.highlightedCode = demoContent.map(d => {
      return {
        ...
        content: ['section'].concat([
          ['pre', { lang: 'jsx', highlighted: Prism.highlight(d.content, Prism.languages['jsx']) },
          ['code', d.content]]
        ])
      };
    });;
  ...
  // 贡献者
  if (meta.contributor) {
    markdownData.contributor = [ 'section',
      [ 'h2', '贡献者' ],
      [ 'table',
        [ 'thead',
          [ 'tr',
        ...
        [ 'tbody',
          [ 'tr',
            [ 'td', '维护者' ],
            [ 'td', meta.contributor.developer || '-' ],
          ],
          [ 'tr',
            [ 'td', '设计者' ],
            [ 'td', meta.contributor.designer || '-' ],
          ]
        ]
      ],
    ];
